...
from tf2_ros import Buffer, TransformListener  # 替换 tf_transformations
...
class PersonDetectionNode(Node):
    ...
    def __init__(self):
        ...
        # 定时发布Marker（10Hz）
        self.create_timer(0.1, self._publish_detection_markers)
    ...
    def _read_pointcloud_fast(self, msg: PointCloud2):
        """快速读取点云并降采样"""
    ...
    def _create_detection_box_marker(self, header, marker_id: int, center_x: float, center_y: float, 
        ...
        # 设置方向 - 手动计算 Z 轴旋转（yaw）到四元数 [x, y, z, w]
        # 替代 tf_transformations.quaternion_from_euler(0, 0, yaw)
        cy = math.cos(yaw * 0.5)
        sy = math.sin(yaw * 0.5)
        marker.pose.orientation = Quaternion(x=0.0, y=0.0, z=sy, w=cy)
