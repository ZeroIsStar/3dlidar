...
class Mmdet3dNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('score_threshold',   0.3)  # 置信度
        ...
        self.declare_parameter('penetration_threshold', 0.7)  # 激光穿透比例阈值，超过此值认为车辆已消失
        self.declare_parameter('penetration_confirm_frames_threshold', 2)  # 连续多少帧确认穿透才删除静态车
    ...
    def _process_pointcloud(self, msg: PointCloud2):
        ...
        for det_idx, track_id in matches:
            ...
            if is_moving:
                ...
                if track.moving_suspect_frames >= 3:
                    ...
                    matched_dyn_id = self._match_dynamic_track(det['center'], self.dynamic_tracks)
        ...
        # ---- 清理未匹配的dynamic tracks ----
        # 删除所有未被本帧匹配到的dynamic tracks（动态车无保护，看不到就消失）
    ...
    def _publish_vehicle_raw_cloud(self, points: np.ndarray, header):
        """发布车辆原始点云（bbox内的原始激光点），颜色设为紫色"""
        if len(points) == 0:
            return
        
        # 使用特定的 intensity 值标记为紫色（在RViz中可根据intensity设置颜色）
        # 这里用 999.0 作为紫色标记值
