...
class Mmdet3dNode(Node):
    ...
    def _process_pointcloud(self, msg: PointCloud2):
        ...
        # ---- 推理 ----
        _t_det_start = time.perf_counter()
        ...
        # ---- 访问result字段 ----
        _t_result_start = time.perf_counter()
        ...
        _t_result_end = time.perf_counter()
        ...
        if _now - self._last_timing_log_time >= 1.0:
            ...
            self.get_logger().info(
                ...
                f'filter={(_t_filter_end-_t_filter_start)*1e3:.1f}ms | '
                f'detector={(_t_det_end-_t_det_start)*1e3:.1f}ms | '
                f'detector_sync={(_t_det_end-_t_det_start)*1e3:.1f}ms | '
                f'result_access={(_t_result_end-_t_result_start)*1e3:.1f}ms | '
